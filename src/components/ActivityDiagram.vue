    ...
    setup(props) {
        ...
        */
        ...
        const conflictPositions = computed(() => {
            ...
            //TODO: implement Error Handling so we can remove this later
            const otherGroups: Conflict[] = [];
            ...
            singlePointGroups.forEach((conflicts, key) => {
                const point = points.value.find(p => p.id === key);
                if (point) {
                    const n = conflicts.length;
                    if (n === 1) {
                        // Ein Konflikt: Render direkt neben dem Punkt (z.B. rechts, also 20 Pixel nach rechts)
                        const finalX = point.x + spacing;
                        const finalY = point.y;
                        result.push({ ...conflicts[0], x: finalX, y: finalY });
                    } else {
                        // Mehrere Konflikte: verteile sie gleichmäßig auf einem Kreis um den Punkt
                        conflicts.forEach((conflict, i) => {
                            const angle = (2 * Math.PI / n) * i; // gleichmäßige Verteilung
            ...
            lineGroups.forEach((conflicts, key) => {
                // Hole die zwei beteiligten Punkte anhand der Typen
                const types = key.split('-');
                const pointA = points.value.find(p => p.id === types[0]);
                const pointB = points.value.find(p => p.id === types[1]);
                ...
                if (pointA && pointB) {
                    ...
                    // Abstand zwischen den Konflikt-Indikatoren (z. B. 20 Pixel)
                    const n = conflicts.length;
                    let offsets: number[] = [];

                    if (n === 1) {
                        // Nur ein Konflikt: Erhalte einen festen, nicht zentralen Offset
                        offsets = [spacing];
                    } else {
                        // Effektive Anzahl = n + 1 (inklusive Dummy-Slot)
                        const effectiveCount = n + 1;
                        // Berechne alle Offsets (symmetrisch um 0)
                        const allOffsets: number[] = [];
                        for (let i = 0; i < effectiveCount; i++) {
                            const offset = (i - (effectiveCount - 1) / 2) * spacing;
                            allOffsets.push(offset);
                        }
                        // Finde den Index des Slots, der am nächsten zum Zentrum (Offset 0) liegt
                        let indexToRemove = 0;
                        ...
                        // Entferne den zentralen Slot
                        allOffsets.splice(indexToRemove, 1);
                        offsets = allOffsets;
                    }

                    // Verteilen: Der mittlere Konflikt liegt genau in der Mitte, weitere werden symmetrisch daneben angeordnet.
                    conflicts.forEach((conflict, i) => {
                        const offset = offsets[i] ?? spacing; // Fallback, falls etwas schiefgeht
            ...
            triangleGroups.forEach((conflicts, key) => {
                const types = key.split('-');
                // Finde die 3 Punkte
                const pts = types.map(type => points.value.find(p => p.id === type)).filter(Boolean);
            ...
            otherGroups.forEach((conflict: Conflict) => {
        ...
        */
        ...
        const handleHover = (event: MouseEvent) => {
            ...
            console.log(foundConflictPoint)
