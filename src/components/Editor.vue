...
export default {
    ...
    computed: {
        titlePlaceholder() {
            return (
                this.staticContent.placeholders.title[this.sessionStore.activeLanguage] ||
                this.staticContent.placeholders.title.en
            );
        },
        pointData() {
            ...
            for (const key in this.activityDetails) {
                if (Array.isArray(this.activityDetails[key])) {
                    pointData[key] = this.activityDetails[key].map((item) => ({
                        id: item.id,
                        label: buildLanguageString(item, this.sessionStore.activeLanguage, true),
                    }));
                } else {
